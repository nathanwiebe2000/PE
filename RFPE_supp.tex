...
\usepackage{xcite}
    \externalcitedocument{RFPE_main}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{amsfonts}
...
\label{app:BCRB}
%=============================================================================
If we assume that Gaussian priors are always used in the inference process then we can use the Bayesian Cramer Rao bound (BCRB) to lower bound the variance on
any unbiased estimate of the unknown phase.  We will see from this analysis that our algorithm is nearly optimal under these assumptions.

If we define $(\Delta \phi)^2$ to be the expected variance of any unbiased estimate of $\phi$ given that $\phi$ is distributed according to a prior distribution $P(\phi)$ then the BCRB states
\begin{equation}
(\Delta \phi)^2 \ge \left(\int I(\phi) P(\phi) \mathrm{d} \phi + \int (\partial_\phi \log(P(\phi)))^2 P(\phi)\mathrm{d} \phi\right)^{-1},
\end{equation}
where $I(\theta)$ is the Fisher information. 

While these expectation values are in general difficult to compute for general Bayesian inference, because we assume that the initial and final distributions are Gaussian we can analytically compute these integrals.  This allows us to lower bound the variance after each measurement and in turn lower bound the variance after $k$ updates to the prior distribution have been applied.  Specifically, under the assumption that $P(E|\phi) = \frac{1+cos(M(\theta-\phi))}{2}$ and $P(\phi) = \exp(-(\phi - \bar{\phi})^2 / 2\sigma^2)/\sqrt{2\pi}\sigma$ then it is easy to see that
\begin{equation}
(\Delta \phi)^2 \ge \frac{\sigma^2}{M^2 \sigma^2 +1}.
\end{equation}
...
\begin{equation}
(\Delta \phi)^2 \ge \frac{\sigma^2}{\kappa^2 +1}.
\end{equation}
Thus after $k$ updates if we assume that the initial distribution is approximately uniform then the uncertainty is at least
\begin{equation}
\Delta \phi_k \ge \frac{\pi}{\sqrt{3(\kappa^2+1)^k}}.
\end{equation}
Setting $\frac{\pi}{\sqrt{3(\kappa^2+1)^k}} = \epsilon$ and inverting this equation gives
\begin{equation}
...
\end{equation}
Through calculus it is then easy to see that the cost is minimized when $\kappa = \frac{1}{2} \sqrt{2 + 2 \sqrt{5}}$.  Substituing this value back into the cost gives the total time needed for the algorithm to be at least
\begin{equation}
\frac{\frac{\sqrt{2 + 2 \sqrt{5}} (1+\sqrt{5}}{2\sqrt{5}-2}}{\epsilon} \approx \frac{3.33}{\epsilon}.
\end{equation}
Our algorithm empirically scales as $4.7/\epsilon$, which suggests that our approach is nearly optimal given the use of Gaussian priors.
