...
Here we address this gap by proposing a new approach to iterative phase estimation that is based on a new approximate inference method that we call \emph{rejection filtering} (RF).  Similarly we call the PE algorithm that follows from RF, \emph{RFPE}.  RFPE is classically efficient, requires a logarithmic number of operations, saturates the Heisenberg bound, 
...
We see in figure that, over $50$ orders of magnitude in the desired error tolerance, this scaling is saturated for RFPE.
