...
\begin{figure}[t!]
\includegraphics[width=0.7\linewidth]{Longpet2.png}
\caption{Error vs evolution time for rejection filter phase estimation for the case where $M$ is continuous and can be interpreted as the experimental time taken.}\label{fig:rms}
\end{figure}
...
This $O(\log(1/\epsilon))$ scaling  is further observed over $100$ orders of magnitude in $\epsilon$ in the appendix.
...
In this case, we expect from the Heisenberg bound that the root-mean-square error should optimally scale as $1/M$.
We see in~\fig{rms} that, over $50$ orders of magnitude in the desired error tolerance, this scaling is saturated for RFPE.
